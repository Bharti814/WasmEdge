...
Expect<AST::CompositeType> Loader::loadCompositeType(TypeCode TC,
                                                     ASTNodeAttr From) {
  switch (TC) {
  case TypeCode::Array:
...
Expect<AST::SubType> Loader::loadSubType(TypeCode TC, ASTNodeAttr From) {
  uint32_t VecCnt = 0;
  if (auto Res = FMgr.readU32(); unlikely(!Res)) {
    return logLoadError(Res.error(), FMgr.getLastOffset(), From);
  } else {
  ...
  if (auto Res = loadCompositeType(TypeCode, From)) {
    return AST::RecursiveType(AST::SubType(*Res));
  } else {
    return Unexpect(Res);
  }
...
Expect<AST::RecursiveType> Loader::loadRecursiveType(ASTNodeAttr From) {
  if (auto CodeByte = FMgr.readByte()) {
    ...
    case TypeCode::Rec: {
      ...
      std::vector<AST::SubType> SList;
      SList.reserve(VecCnt);
      for (uint32_t I = 0; I < VecCnt; ++I) {
        if (auto Res = loadSubType(From)) {
          SList.push_back(*Res);
        } else {
          return Unexpect(Res);
        }
      }
      return AST::RecursiveType(SList);
    }
    case TypeCode::Sub:
    case TypeCode::SubFinal:
      if (auto Res = loadSubType(Code, From)) {
        return AST::RecursiveType(*Res);
      } else {
        return Unexpect(Res);
      }
    case TypeCode::Array:
